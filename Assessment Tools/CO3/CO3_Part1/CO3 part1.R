...
source_files <- data.frame(
  ...
  Asset_ID = paste0("A", sprintf("%02d", 1:15)),
...
cat("SOURCE CODE ASSETS SCANNED\n")
cat("------------------------------------------------------------\n")
print(source_files)
...
sast_findings <- data.frame(
  ...
  Asset_ID = source_files$Asset_ID[
    match(
      selected_files,
      source_files$File_Name
    )
  ...
  Asset_Name = source_files$Asset_Name[
    match(
      selected_files,
      source_files$File_Name
    )
...
cat("\nTotal Assets Scanned      :", nrow(source_files))
...
write.csv(
  ...
  "CO3_PART1_Vulnerability_Summary.csv",
  row.names = FALSE
)

write.csv(
  file_summary,
  "CO3_PART1_File_Wise_Summary.csv",
  row.names = FALSE
)
